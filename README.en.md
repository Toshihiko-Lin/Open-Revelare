...
- **Density-domain inversion** — base `t_base`, the per-channel highlight density `d_max_per_channel` (the white end), the per-channel shadow density (the black end), scan exposure. Every one is adjustable in the UI and physically meaningful. The inversion is a **two-endpoint model**: both ends are that channel's own measured density, the slope follows from their difference, and the between-channel differences **are** the white balance — it is the endpoints themselves, not a stage applied after them. There is no separate gamma, chroma, or `wb_high` correction factor (earlier versions had `grade` / `pivot` / `chroma_grade` / `wb_high`; all are gone)
