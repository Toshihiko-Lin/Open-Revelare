...
| D-036 | Accepted（amends D-032；2026-09-18 实机报告驱动） | **HDR 下 Stage 2：按跨度定义的两项跨到上限，按锚点定义的三项锚在 SDR 白。** 载体不再除以 headroom，而是以 diffuse white = 1 直接进 `Srgb.LinearToSrgbExtended`（SDR 编码向上延续）；`ApplyOperationChain` 新增 `rangeTop`（SDR 为 1，扩展目标为编码后的 headroom）：高光阴影与曲线在 `[0, rangeTop]` 上定义（曲线右端仍是 HDR 峰值、高光滑块仍压得到 SDR 白以上），反差仍绕 0.5（SDR 中灰）、色阶仍以 0/1 为黑白，仿射向上延续；扩展路径上这两项对**进来非负**的分量不得压成负（`KeepSign`），D-005 的域外负分量照旧穿过。同时 `HighlightRolloff.BoundAbove` 与 `HighlightSoftProof.Fit` 改为**按像素**：以最大分量定缩放系数，整色等比缩到顶，色相与饱和度不变。SDR 路径逐位不变（`rangeTop = 1` 时算式恒等） | 用户 2026-09-18 在 HDR 显示器上实测 D-032：拉反差整卷"爆红蓝"，拉曝光高光偏色。原因是按 headroom 归一后反差的枢轴落在 diffuse white（headroom 4.9 时中灰编码 0.21、白 0.48），整幅画面都在枢轴之下被逐通道压暗——+0.3 时暗部 0.15/0.12/0.10 变 0.07/0.03/0.008，R 是 B 的九倍；且扩展路径不钳负，反差把正分量压成负即制造域外色，广色域面板显示为饱和红蓝。高光偏色则来自逐通道的顶封（`BoundAbove` 硬裁）与逐通道软校样：曝光把高光推过顶后，R 先到顶、G 次之，色相在途中改变。两类操作性质不同：反差与色阶由**锚点**定义（中灰、黑白），锚点不应随上限移动——Lightroom 的 HDR 反差同样绕中灰；高光阴影与曲线由**跨度**定义，D-032 让它们跨到上限的理由（"压不到 HDR 高光的高光滑块不是高光滑块"）仍成立。D-032 明说的代价（同参数在两目标上 SDR 段影调不同、由增益图承载）现只剩这两项承担。顶封与软校样按像素而非逐通道，与 D-034 "为何用标量而不逐通道"同一理由：HDR 的顶是亮度不是纸白，一个颜色可以坐在顶上而不是白 |
...
| 2026-09-18 | `main`，续于 `d05a2a5`，工作区改动待提交 | **D-036：HDR 下反差／色阶锚在 SDR 白，高光阴影／曲线跨到上限；顶封与软校样按像素保色相**：`Stage2.ApplyOperationChain` 加 `rangeTop`（默认 1）与 `KeepSign`；`ApplyManagedToExtendedTarget` 不再除以 headroom，改传 `rangeTop = LinearToSrgbExtended(headroom)`；`HighlightRolloff.BoundAbove` / `HighlightSoftProof.Fit` 改按最大分量整色缩放；`HdrLimitHint` 文案改为只有高光阴影与曲线随上限（en.json 同步）。同批修的 GUI 项：`WbMath.ETint` 取反（+ 为洋红，对齐轨道色与 LR）；`PushLevel` 三分量写入压住渲染只渲一次（拖 D_max 红蓝闪动即三帧中间态）；逐通道密度滑条轨道改为色→补色 | `-warnaserror` 0/0；managed 690 全过（新增：三项锚定操作在拐点以下与 SDR rendition 一致、反差不制造负分量、`BoundAbove` 保比例与 NaN 邻居仍封顶、软校样整色缩放）、Win32 40 全过。**未实机** | 用户实机：HDR 卷拉反差看暗部是否仍爆红蓝、拉曝光看高光是否仍偏色；确认后进 v1.7.3 |
