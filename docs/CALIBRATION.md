...
`C41Crosstalk.cs` 把 `chroma_grade` 的各向同性标量换成「通用方向矩阵 × 强度标量」，`FrameParams.UseC41Crosstalk` 开关，**默认关闭**。

**依据**（`calibration/universal_crosstalk.py`）：对 DiVERE 的八套 C-41 模拟色卡逐卷求解密度色度域 3×3，归一化强度后八卷方向的余弦相似度 **0.9957–0.9997**；给每卷独立矩阵相比「共享方向 + 逐卷标量」只好 **0.1%**。方向属于工艺，强度属于胶片——正是要保留的风格差异（Portra 160 4.21 → Gold 200 3.45 → DIR 变体 3.16，DIR 压强度而不转方向）。

矩阵行和为 0（1e-16），只动色度不动亮度，与管线中 `c0,c1,c2` 为零和分量的结构吻合。

**但实测收益远小于预期。** 在矩阵被拟合的那个域里，与「每卷最优标量」相比：

```
kodak_ektar_100      0.43246 → 0.41448   4.2%
kodak_gold_200       0.44886 → 0.42389   5.6%
kodak_portra_160_DIR 0.41759 → 0.41270   1.2%
kodak_portra_400     0.43000 → 0.41022   4.6%
...
平均                 0.43329 → 0.41659   3.9%
```

**只好 3.9%。** 我此前依据"标量补不平（per-patch 误差 max 0.206）"推断矩阵会带来大幅改善，那个推断过头了。

更要紧的是残差绝对值仍有 **0.42**——主要误差不在色度变换这一层，换成矩阵解决不了。也就是说：矩阵在结构上正确、在原理上该用，但它**不是** `chroma_grade` 依据薄弱这个问题的答案。真正的大头仍未定位。

因此它以默认关闭的形式并入：结构正确、零风险、可复现，供后续在真实底片上继续验证，而不是当作已解决的问题。
