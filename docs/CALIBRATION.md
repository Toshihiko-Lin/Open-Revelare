...
因此改为从相机自身的色彩数据取矩阵，**在解码时施加** —— 和扫描仪路径完全对称。

### 位置：只能在解码器里

这一点是试错试出来的，值得写清楚。**扫描仪路径从一开始就是对的**：`TiffIO` 在读取时就施加 ICC 的 rXYZ/gXYZ/bXYZ 矩阵，所以扫描件进入管线时**已经是线性 sRGB**，`t_base` 随后在已表征的数据上采样。关键不是"矩阵放哪一步"，而是 **表征发生在标定之前**，于是 `t_base` / `wb_high` / `wb_offset` / `d_max` 四个测量天然处在同一个空间。

把相机矩阵放进管线中间则**两种放法都失败**，都在真实底片上验证过：

- **反相之后**：65.6% 的真实底片颜色被压成负值，钳制即改色相，红被推向品红（~1-5° → ~352-357°）。用户报告"红色失真"。
- **反相之前**：密度标尺直接崩溃。即使把 `t_base` 一起变换也不够 —— `wb_high`/`wb_offset`/`d_max` 仍指向旧空间。用户报告"色彩大幅漂移且溢出，完全不可用"。

**移动矩阵不是调整顺序，是更换坐标系**，而整个坐标系一次性改变的地方只有解码器。

实现：`RawDecode.CharacteriseInput` 开启时，LibRaw 认识的机身走它自己的 `output_color = 1`；不认识的（如 OM-5，实测 0 与 1 输出逐字节相同）在解码后用 `CameraMatrixFallback` 补上。两条路都在 `Process()` 这个唯一收口处理，因此全帧解码、区域切片、降采样预览、Path A 标定 ROI 均一致。

- **中性轴不动**：两种矩阵行和均为 1，白点原样保留。
...
**迁移**：`roll_meta.characterise_input`，文件里没有这个字段就是 `false`。表征改变的是**标定基准本身**（`t_base` 现在在表征后的空间采样），因此**旧工程默认关闭**、保持原样，新建卷默认开启。用户可在「输入色彩表征」面板自行切换；切换后建议重新采样片基。

实测（OM-5，同一张底片）：`t_base` 从 `0.5578/0.9835/0.5874` 变为 `0.5043/1.0000/0.4246` —— 正是"在表征后的数据上采样"的体现。两种模式下渲染均无越界、无 NaN。
