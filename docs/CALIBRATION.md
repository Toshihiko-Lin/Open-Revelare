...
`C41Crosstalk.cs` 把 `chroma_grade` 的标量换成「方向矩阵 × 强度」，`FrameParams.UseC41Crosstalk` 开关，**默认关闭**。代码结构是对的，但**它所依据的数据不适用于本管线**，这一节记录为什么。

### 相纸在负片一侧，不在目标一侧

我先前以为 DiVERE 那批数据的"相纸"出现在目标里，查证后是反的：

```
目标侧  original_color_cc24data.json
        "ColorChecker 24 色块原始测量值在 XYZ 色彩空间的数值"
        required_working_colorspace: None          ← 真实场景，无相纸 ✓

负片侧  kodak_*_kodak_endura_premier_d60_cc24data.json
        "…→kodak_endura_premier_d60_uc 打印流程下，相纸上得到的理论密度"
        required_working_colorspace: KodakEnduraPremier   ← 含相纸 ✗
```

所以拟合出的矩阵映射的是 **相纸密度 → 场景**，而本管线需要的是 **底片密度 → 场景**，两者相差整条印相链路 —— 而印相链路正是本软件明确不引入的东西（相纸观感只在软打样/导出色域里作为**模拟**提供，不进入还原路径）。

**这也解释了那个 0.42 的残差**：其中相当一部分是在拟合印相链路，不是胶片串扰。此前把它归因于"相纸链路未建模"不够准确 —— 不是未建模，是**建模方向反了**。

### 因此撤回的结论

- **"C-41 存在通用串扰方向"** —— 证据被削弱。八卷方向高度一致（余弦 0.9957–0.9997）是真的，但它们共用同一条印相链路，一致性有多少来自胶片、多少来自相纸，无法从这批数据分离。**此问题保持开放。**
- **`C41Crosstalk.Direction` 不是工艺常数** —— 它含 Endura 相纸的染料特性，不应以"C-41 通用解"的名义使用。

站得住的部分：拟合方法本身、矩阵秩为 2（亮度零空间，行和 1e-16）、以及 DIR 变体只压强度不转方向（3.15–3.17 vs noDIR 3.43–4.06）—— 最后这条较难用相纸解释。

### 强度不可能是工艺常数

顺带确认了一件独立于数据源的事：强度在结构上等于

```
strength = 目标色度 / 负片色度
```

实测中 target chroma 恒为 1.1096（同一张色卡），变化的全是分母。**它由"把什么声明为目标"决定，不是 C-41 的物理属性** —— 所以无论取 3.05、3.61 还是 3.86，都摆脱不了"来自某组特定样本"。追求"通用强度标量"这个方向本身不成立。

### 现状

矩阵默认关闭，不影响任何实际输出。要让这条路走通，需要**不含相纸的胶片密度数据**，DiVERE 未提供。在那之前，`chroma_grade` 保持原样：依据薄弱（见上文），但至少不破坏密度关系。
