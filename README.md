...
- **Why this project** — born from the author's own camera-scanning workflow. The mainstream options are Adobe plugins with opaque, unreproducible processing, and the community calls it "dark magic". OpenRevelare treats the mask as physics: Cineon density-domain math, named parameters, reproducible results. Validated with real paying users, rewritten in C#, open-sourced in August 2026.
- **Quick start** — import your scans, let the auto-calibration estimate the film base, sync the physical parameters to the whole roll, fine-tune each frame in the edit stage, export. No save button; everything is written automatically to a `.ncproj` next to your images.

- **Website** — [revelare.netlify.app](https://revelare.netlify.app/) (Chinese), with screenshots of the library, develop and contact-sheet views.
- **Download** — [Releases](https://github.com/Toshihiko-Lin/Open-Revelare/releases/latest). Windows x64 `setup.exe` (stable), Linux x86_64 `.AppImage` (beta, glibc ≥ 2.35), macOS Apple Silicon `.dmg` (beta, never run on real hardware). The .NET runtime is bundled. Packages are unsigned and the macOS build is not notarised; see the install notes above.
- **Input** — RAW via LibRaw (DNG / NEF / CR2 / CR3 / ARW / RAF / RW2 / ORF / PEF / IIQ …), plus TIFF, JPEG and PNG. **Output** — 8/16-bit TIFF and JPEG, with an embedded sRGB or Adobe RGB ICC profile.
- **Build** — .NET 8 SDK, then `dotnet build -c Release` and `dotnet run --project src/OpenRevelare.Gui`. A CLI front-end shares the same Core.
- **Licence** — GPL-3.0-only, **except** `models/net_awb.onnx`, which is a third-party asset under CC BY-NC-SA 4.0 and is therefore **not** covered by the GPL grant. Delete `models/` before any commercial redistribution; everything but the "smart white balance" feature keeps working. See [models/README.md](models/README.md) and [THIRD_PARTY_NOTICES.txt](THIRD_PARTY_NOTICES.txt).
