...
- 只想一键出片、完全不打算理解任何参数：**自动标定能带你走到 90%，而这个工具的价值在于剩下的 10% 是可检查的，而不是靠猜**。不在乎那 10% 的话，用别的更省事
...
说得更直白些：**自动标定能带你走到 90%，剩下的 10% 才是这个工具存在的理由**——不是因为它非要你手动，而是因为那 10% 在这里是**可检查的**。片基读数是三个能看懂的数（橙罩必然 R<G<B），高光端点是三个能看懂的数，差一点在哪里、该往哪边改，都有据可依，不是「再拉拉曲线试试」。别的方案不是没有那 10%，是没法知道它去哪了。
...
- **密度域反转**——**恰好六个自由度**：片基 `t_base` 之上，每端三个通道的绝对密度（`d_min_per_channel` 黑端、`d_max_per_channel` 白端）。这正是渲染消费的自由度数，一一对应。整体明暗＝两端同步升降，反差＝两端拉近拉远，白平衡＝通道间之差——三者都是这六个数的不同读法，**不存在独立的亮度、反差、gamma、色度或白平衡参数**（`grade` / `pivot` / `chroma_grade` / `wb_high` / `wb_offset` / `d_max` / `scan_ev` 均已移除）
...
dotnet test  -c Release          # 回归测试，约 10 秒
dotnet run --project src/OpenRevelare.Gui
```

想改代码的话，先读一下 [CONTRIBUTING.md](CONTRIBUTING.md)——尤其是关于**金标准基线**的那一节：`src/OpenRevelare.Tests/Golden/` 里存着整条管线的逐像素基线，它守的就是上面说的「今天处理、明年处理、换台机器处理，结果都一样」。它红了通常意味着你无意中改变了每一张照片。
