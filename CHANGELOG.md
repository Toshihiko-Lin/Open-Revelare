...
- **导出新增文件名命名，长边尺寸精确落位**。从六种现成命名里选一个（原文件名、卷名 + 序号、冲洗日期 + 卷名 + 序号等），旁边即时显示这一帧的实际文件名，要别的可选「自定义…」按字段拼；单张导出据此预填保存对话框的名字。尺寸可选择允许放大。
...
- **导出弹窗重排**。分节成卡片、控件对齐同一列，不适用的选项直接隐藏而不是置灰，提示文字收短。窗口不再超出屏幕，Esc 关闭、Enter 确认。
...
- **界面更新：右侧面板参数组卡片化、滑条带方向色，Cineon 三步标了 ①②③；导出窗口只显示所选格式的选项，提示文字改成人话**。

- **JPEG 导出可以限定文件不超过多少 MB**。导出窗口「JPEG 品质」下勾选「限制文件大小」并填一个数；超限时先从所选品质往下降（最低 40），仍超限再按整数倍缩小尺寸，实际用到的品质、尺寸和大小在状态栏报告。整卷导出逐帧各自守限。TIFF 无品质可降，选 TIFF 时不显示。

**修复**

- **Windows 上拖裁切框、拉平线、滑块时预览不再等鼠标停下才更新**。拖动中合成好的帧此前被当作过期丢弃；现在算完即上屏，全屏裁切拖动每步从 ~150 ms 到 ~20 ms。macOS 同路径一并修复，实机待验。

- **RGB 分光（Path A）导入的卷自动结果不再偏黄偏红**。色度补偿矩阵改在端点归一后作用，标定的黑白两端严格中性；白光/扫描卷不受影响，旧工程重新打开即生效。
...
- **UI refresh: the right-hand panel groups become cards with direction-coloured sliders and the Cineon steps are numbered ①②③; the export dialog shows only the chosen format's options, with its hints rewritten in plain language.**

- **A JPEG export can be held under a size in MB.** Tick “Limit file size” under JPEG quality in the export dialog and enter a number; when the file is over, quality is lowered first (down to 40), and only if that is still too big is the picture shrunk by whole integer factors. The quality, size and bytes actually used are reported in the status bar, and a roll export holds every frame to the limit individually. TIFF has no quality to give up, so the option is not shown for it.

**Fixed**

- **The preview no longer waits for the pointer to stop while dragging the crop frame, straighten line or sliders on Windows.** Frames composed mid-drag were being discarded as stale; they are now presented as soon as they finish, and a full-screen crop drag went from ~150 ms a step to ~20 ms. macOS shares the fix; not yet verified on hardware.

- **RGB narrow-band (Path A) imports no longer come out yellow-red.** The chroma-compensation matrix now acts after endpoint normalisation, so both calibrated ends stay neutral; white-light/scanner rolls are unaffected, and saved projects pick it up on reopen.
