...
- **修复了 Windows 上拖裁切框、拉平线、拉滑块时预览要等鼠标停下才更新的问题**。每次指针移动都会抓一份新快照并推进版本号，而一帧合成需要 ~20 ms，于是拖动中算完的每一帧一比版本号都"过期"被丢弃，只有停手后的最后一帧能上屏。现在算完即上屏（只有视口尺寸变了的帧才让位给新帧），并且上屏不再排队等 UI 线程。同时把这条路径整体提速：裁切/拉线只重画框线不重缩放图像、打包和逐像素循环全部并行、拖动中不再每帧分配两块 18 MB 的大数组（全屏时 GC 停顿的来源）。全屏裁切拖动从每步 ~150 ms 到 ~20 ms。刚打开一卷后的几十秒里后台预热仍会占满 CPU，那段时间的拖动会慢一些。macOS 走同一条合成路径，丢帧的根因一并修复；上屏仍在 UI 线程排队，尚未在实机验证是否已足够跟手。
...
- **Fixed the preview on Windows only updating once the pointer stopped while dragging the crop frame, the straighten line or a slider.** Every pointer move captured a snapshot and advanced an epoch, and a frame took ~20 ms to compose, so every frame finished mid-drag compared as "stale" and was dropped; only the frame composed after the last move was ever shown. Finished frames are now presented at once (only a frame for a viewport size that has since changed yields to the next), and the present no longer waits for a UI-thread turn. The path is also much cheaper: crop and line drags re-rasterise only the geometry over a cached fitted picture, packing and every per-pixel loop run in parallel, and a drag no longer allocates two 18 MB arrays per frame (the source of the full-screen GC stalls). A full-screen crop drag went from ~150 ms a step to ~20 ms. For the first tens of seconds after opening a roll the background warm-up still saturates the CPU, and drags in that window remain slower. macOS shares the composition path, so the frame-dropping root cause is fixed there too; its present still queues on the UI thread and has not yet been verified on hardware to follow the pointer.
...
- **TIFF 导入快得多**。扫描 TIFF 改为多线程解码，预览、分格、导出一起受益；嵌了 ICC 的 TIFF 预览另走色彩引擎的快速路径，只用于预览与测量，导出仍是精确变换。一条 127 MP 的 Flextight 扫描预览从 2.5 s 到 0.6 s，121 MP 的 Adobe RGB 导出件从 49 s 到 0.9 s（12 核）。
...
- **Much faster TIFF import.** Scanner TIFFs are decoded on all cores, which speeds up previews, split cells and exports alike; previews of ICC-tagged TIFFs also take the colour engine's fast path, used for previews and measurements only — exports still use the exact transform. A 127 MP Flextight strip previews in 0.6 s instead of 2.5 s, and a 121 MP Adobe RGB export in 0.9 s instead of 49 s (12 cores).

**Fixed**

- **Fixed sprocket detection failing on a whole roll when the light board is unevenly bright.** 

- **Fixed blown-white or off-colour results when splitting large scans on a small-memory machine.** Every split cell's preview used to decode the whole file before cropping. Previews, split cells, exports and the sharp zoom now decode only their own cell, bit-identical to the whole-frame decode, and frames that fail to decode are reported in the status line.
