...
- **修复了小内存机器分条导入大幅扫描时爆白或整卷偏色的问题**。此前每一格的预览都要整张解码再裁，一条 Flextight 整条扫描就是 1.5–2.3 GB，8 GB 内存的机器上要么自动流程起不来（负片一片白），要么整卷分析里几格解码失败被无声丢掉，wb_high 每次导入都不一样。现在预览、分格、导出和局部放大都只解码自己那一格，结果与整张解码逐位相同；解码失败的帧会写进状态栏。
...
- **Fixed blown-white or off-colour results when splitting large scans on a small-memory machine.** Every split cell's preview used to decode the whole file before cropping — 1.5–2.3 GB for a whole Flextight strip — so on an 8 GB machine the auto chain either never started (a white negative) or the roll-wide analysis silently dropped the cells that failed to decode, giving a different wb_high on every import. Previews, split cells, exports and the sharp zoom now decode only their own cell, bit-identical to the whole-frame decode, and frames that fail to decode are reported in the status line.
