...
- **修复了 Windows 上拖裁切框、拉平线、拉滑块时预览要等鼠标停下才更新的问题**。每次指针移动都会抓一份新快照并推进版本号，而一帧合成需要 ~20 ms，于是拖动中算完的每一帧一比版本号都"过期"被丢弃，只有停手后的最后一帧能上屏。现在算完即上屏（只有视口尺寸变了的帧才让位给新帧），并且上屏不再排队等 UI 线程。同时把这条路径整体提速：裁切/拉线只重画框线不重缩放图像、打包和逐像素循环全部并行、拖动中不再每帧分配两块 18 MB 的大数组（全屏时 GC 停顿的来源）。全屏裁切拖动从每步 ~150 ms 到 ~20 ms。刚打开一卷后的几十秒里后台预热仍会占满 CPU，那段时间的拖动会慢一些。
